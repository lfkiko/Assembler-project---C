...
int main(int argc, char* argv[]) {
	...
	int i , j, first_op_val = -1 , second_op_val = -1;
	int IC, DC, LC;	
		...
		if(READ_fp == NULL)	{
			...
			while (fgets(line, LINE_LENGTH, READ_fp) != NULL) {										/*beginning of the forst reading*/
				...
				if((line[i] != ';') && strcmp(line,"\n")) {											/*if line is blank or start with ';'*/
					...
					FUNCTION_CHECK(j, &TEMP, &LABEL, &DIRECTIVE, &FIRST_OP, &SECOND_OP, &OPCODE_FUNCT, &OBJ_HEAD, &assembler, &LC, &IC, &DC, first_op_val,
					second_op_val);	
